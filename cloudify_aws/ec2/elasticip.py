...
@operation
def allocate(**_):
    return ElasticIP().allocated()


@operation
def release(**_):
    return ElasticIP().released()
...
class ElasticIP(AwsBaseNode):
    ...
    def allocate(self, **_):
        ...
        if self._allocate_external_elasticip():
            return
    ...
    def allocated(self):
        ...
        ctx.logger.info(
                'Attempting to allocate {0} {1}.'
                .format(self.aws_resource_type,
                        self.cloudify_node_instance_id))

        if self.allocate():
            return self.post_allocate()

        raise NonRecoverableError(
                'unable to allocate this resource.')

    def post_allocate(self):

        ctx.instance.runtime_properties['public_ip_address'] = self.resource_id
    ...
    def release(self, **_):
        ...
        if self._release_external_elasticip():
            return
    ...
    def _release_external_elasticip(self):
    ...
    def _allocate_external_elasticip(self):
        ...
        address_ip = self._get_address_by_id(
                ctx.node.properties['resource_id'])
        if not address_ip:
            raise NonRecoverableError(
