...
class SecurityGroup(AwsBaseNode):
    ...
    def create(self, **_):
        ...
        if self._create_external_securitygroup(name):
            return
        ...
        security_group = self._get_security_group_from_id(
    ...
    def _get_security_group_from_id(self, group_id):
    ...
    def _get_all_security_groups(self, list_of_group_names=None,
        ...
        for rule in ctx.node.properties['rules']:
            ...
            if 'src_group_id' in rule:
                ...
                if not group_object.vpc_id:
                    src_group_object = \
                        self._get_security_group_from_id(
                                rule['src_group_id'])
                else:
    ...
    def _get_vpc_security_group_from_name(self, name):
        groups = self._get_all_security_groups()
        for group in groups:
            if group.name == name:
                return group
        return None
    ...
    def _delete_security_group(self, group_id):
        ...
        group_to_delete = self._get_security_group_from_id(group_id)
    ...
    def _create_external_securitygroup(self, name):
        ...
        group = self._get_security_group_from_id(name)
        if not group:
            raise NonRecoverableError(
