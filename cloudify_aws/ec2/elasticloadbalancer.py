...
def add_instance_to_elb(**_):
    return ElbInstanceConnection().added()


@operation
def remove_instance_from_elb(**_):
    return ElbInstanceConnection().removed()
...
class ElbInstanceConnection(AwsBaseRelationship):
    ...
    def remove_instance_from_elb(self, **_):
    ...
    def removed(self):
        ...
        ctx.logger.info(
                'Attempting to deregistrate {0} from {1}.'
                .format(self.source_resource_id,
                        self.target_resource_id))

        if self.remove_instance_from_elb():
            return self.post_removed()

        raise NonRecoverableError(
                'Neither external resource, nor Cloudify resource, '
                'unable to deragistrate this resource.')

    def post_removed(self):
